        ...
        <tbody>
          {projects.map((proj) => (
            <tr key={proj.project_id} className="border-b last:border-b-0">
              <td className="px-4 py-2 text-right align-top max-w-[240px] break-words">
                {renderVal(proj.title + (proj.group_name ? ` — ${proj.group_name}` : ''))}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal(proj.type)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal(proj.state)}
              </td>
              <td className="px-4 py-2 text-right align-top max-w-[320px] break-words">
                {renderVal(proj.description)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal(proj.supervisor?.name)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal(proj.co_supervisor?.name)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal((proj as any).college_name)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal((() => {
                  const p: any = proj as any;
                  if (p.university_name) return p.university_name;
                  try {
                    const col = p.college && typeof p.college === 'object' ? p.college : null;
                    if (col) {
                      if (col.uname_ar || col.name_ar) return col.uname_ar || col.name_ar;
                      if (col.branch_detail && col.branch_detail.university_detail) {
                        const ud = col.branch_detail.university_detail;
                        return ud.uname_ar || ud.name_ar || ud.uname_en || ud.name || '-';
                      }
                      if (col.university && typeof col.university === 'object') {
                        return col.university.uname_ar || col.university.name_ar || col.university.uname_en || col.university.name || '-';
                      }
                    }
                  } catch (e) { /* ignore */ }
                  return '-';
              ...
              <td className="px-4 py-2 text-right align-top">
                {renderVal((proj as any).department_name)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal((() => {
              ...
              <td className="px-4 py-2 text-right align-top">
                {renderVal((proj as any).field?.name || (proj as any).field)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal((proj as any).program_name || ((proj as any).program && ((proj as any).program.p_name || (proj as any).program.name)))}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal(Array.isArray((proj as any).tools) ? (proj as any).tools.join(', ') : (proj as any).tools)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal((proj as any).created_by?.name || (proj as any).created_by_name || (proj as any).created_by)}
              </td>
              <td className="px-4 py-2 text-right align-top">
                {renderVal((() => {
