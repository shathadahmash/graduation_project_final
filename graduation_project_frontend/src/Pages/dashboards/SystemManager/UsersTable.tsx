...
const UsersTable: React.FC = () => {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(u => {
      ...
      const matchesSearch =
        ...
        (u.CID?.toLowerCase().includes(q) ?? false); // include CID in search
      const matchesRole =
  ...
  const openCreate = () => {
    setEditingUser(null);
    setForm({ first_name: '', last_name: '', username: '', email: '', phone: '', gender: '',CID: '' });
    setSelectedRoles([]);
    setShowModal(true);
  };
              ...
              {filteredUsers.length ? (
                filteredUsers.map((u, idx) => (
                  ...
                  >
                    ...
                    <td className="border border-black px-4 py-2">{u.CID || '-'}</td> {/* show CID */}
          ...
          <div className="bg-white w-full max-w-lg p-6 rounded-lg shadow">
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-3 flex-wrap">
                {roles.map(r => (
                  <label key={r.id} className="flex gap-1 text-sm">
                    <input
                      type="checkbox"
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button onClick={() => setShowModal(false)} className="border px-4 py-1 rounded">
                إلغاء
              </button>
