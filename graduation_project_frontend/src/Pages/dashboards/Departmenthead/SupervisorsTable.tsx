...
const SupervisorsTable: React.FC = () => {
  ...
  const getDepartmentHeadDepartmentId = async (userId: number): Promise<number | null> => {
    try {
      ...
      console.error('[SupervisorsTable] Failed to fetch department head department:', err);
  ...
  useEffect(() => {
    const fetchSupervisors = async () => {
      try {
        ...
        // Filter supervisors by department head's department using their affiliations
        if (deptHeadDepartmentId) {
          ...
          setSupervisors(filteredByDepartment);
        } else {
