...
const StudentTable: React.FC = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Create a map of user_id to department_id from affiliations
      ...
      // Get department head's department ID from their AcademicAffiliation
      let deptHeadDepartmentId = userDepartmentMap.get(currentUser?.id || 0);
      if (!deptHeadDepartmentId && currentUser?.department_id) {
        deptHeadDepartmentId = currentUser.department_id;
      }

      console.log('Department Head Department ID:', deptHeadDepartmentId);
      console.log('Current User:', currentUser);
      console.log('User Department Map:', userDepartmentMap);
      ...
      const departmentStudents = usersData.filter(user => {
        ...
        console.log(`User ${user.id} (${user.name}): department=${userDepartmentId}, isStudent=${isStudent}, isSameDepartment=${isSameDepartment}`);
        
        return isStudent && isSameDepartment && userDepartmentId !== undefined;
      });

      // Attach affiliation data to each student
      const studentsWithAffiliations = departmentStudents.map(user => {
