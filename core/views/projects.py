...
class ProjectFilter(django_filters.FilterSet):
        ...
        return queryset.filter(
            Exists(
                GroupSupervisors.objects.filter(
                    group__project=OuterRef("pk"), user__id=value, type="supervisor"
                )
            )
        )
    ...
    def filter_co_supervisor(self, queryset, name, value):
        return queryset.filter(
            Exists(
                GroupSupervisors.objects.filter(
                    group__project=OuterRef("pk"), user__id=value, type="co_supervisor"
                )
            )
        )
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        try:
            total = qs.count()
            logger.debug(
                "ProjectViewSet: total projects before role filtering: %s", total
            )
        except Exception:
            logger.debug("ProjectViewSet: unable to count total projects")

        # ---- External users only see their projects ----
        is_external = UserRoles.objects.filter(
            user=user, role__type__icontains="External"
        ).exists()

        if is_external:
            logger.debug(
                "ProjectViewSet: user %s is external - returning own projects", user
            )
            return qs.filter(created_by=user)

        # ---- Students can view all projects ----
        if PermissionManager.is_student(user):
            logger.debug(
                "ProjectViewSet: user %s is student - returning all projects for student",
                user,
            )
            return qs

        # ---- Admin (not dean) sees all projects ----
        if PermissionManager.is_admin(user) and not PermissionManager.is_dean(user):
            logger.debug(
                "ProjectViewSet: user %s is admin (not dean) - returning all projects",
                user,
            )
            return qs
        ...
        if PermissionManager.is_supervisor(user):
         return qs.filter(
         Q(groups__groupsupervisors_set__user=user) | Q(created_by=user)
         ).distinct()

        # ---- Dean sees projects belonging to their colleges ----
        if PermissionManager.is_dean(user):
            college_ids = AcademicAffiliation.objects.filter(user=user).values_list(
            ...
            logger.debug(
                "ProjectViewSet: user %s is dean - affiliated colleges: %s",
                user,
                college_ids,
            )
            ...
            if college_ids:
                ...
                try:
                    ...
                    sample_ids = list(
                        filtered_qs.values_list("project_id", flat=True)[:20]
                    )
                    logger.debug(
                        "ProjectViewSet: dean filtered projects count=%s sample_ids=%s",
                        cnt,
                        sample_ids,
                    )
                except Exception:
        ...
        logger.debug(
            "ProjectViewSet: user %s matched no specific role rule - returning all projects",
            user,
        )
    ...
    def create(self, request, *args, **kwargs):
            try:
                ...
                if "state" not in data:
                    pending_state, _ = ProjectState.objects.get_or_create(
                        name="Pending"
                    )
    ...
    def filter_options(self, request):
            try:
                ...
                university_list = [
                    {"id": u["uid"], "name": u["uname_ar"]} for u in universities
                ]

                colleges = College.objects.values("cid", "name_ar")
                college_list = [
                    {"id": c["cid"], "name": c["name_ar"]} for c in colleges
                ]

                active_supervisors = (
                    User.objects.filter(groupsupervisors__type="supervisor")
                    .distinct()
                    .values("id", "name")
                )
                supervisor_list = [
                ...
                active_co_supervisors = (
                    User.objects.filter(groupsupervisors__type="co_supervisor")
                    .distinct()
                    .values("id", "name")
                )
                co_supervisor_list = [
                ...
                years_qs = (
                    Project.objects.values_list("start_date", flat=True)
                    .distinct()
                    .order_by("-start_date")
                )
                ...
                # Get distinct tools
                tools_qs = (
                    Project.objects.values_list("tools", flat=True)
                    .distinct()
                    .exclude(tools__isnull=True)
                    .exclude(tools__exact="")
                )
                tools = [str(t) for t in tools_qs if t is not None and t.strip()]

                # Get distinct fields
                fields_qs = (
                    Project.objects.values_list("field", flat=True)
                    .distinct()
                    .exclude(field__isnull=True)
                    .exclude(field__exact="")
                )
    ...
    def my_project(self, request):
        ...
        project = Project.objects.filter(groups__groupmembers__user=user).first()
    ...
    def update_project(self, request, pk=None):
        ...
        if not user_can_edit and PermissionManager.has_permission(
            user, "change_project"
        ):
            user_can_edit = True
        ...
        if not user_can_edit and PermissionManager.is_dean(user):
            ...
            if college_ids:
                if Group.objects.filter(
                    project=project,
                    program_groups__program__department__college__in=college_ids,
                ).exists():
                    user_can_edit = True
    ...
    def delete_project(self, request, pk=None):
        ...
        if PermissionManager.has_permission(user, "delete_project"):
            ...
            user_affiliation = (
                AcademicAffiliation.objects.filter(user=user)
                .order_by("-start_date")
                .first()
            )
...
def dropdown_data(request):
    ...
    user_affiliation = (
        AcademicAffiliation.objects.filter(user=user).order_by("-start_date").first()
    )
    ...
    if PermissionManager.is_student(user) and user_department:
        ...
        students = (
            User.objects.filter(userroles__role__type="Student")
            .exclude(id=user.id)
            .distinct()
        )
